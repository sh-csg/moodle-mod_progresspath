...
export const init = () => {
    ...
    Templates.prefetchTemplates(['mod_progresspath/cssskeleton']);
    ...
    let mapdiv = document.getElementById('progresspath-editor-map');
    let code = document.getElementById('id_svgcode');

    // DOM nodes for the activity selector
    let activitySetting = document.getElementById('progresspath-activity-setting');
    let activitySelector = document.getElementById('progresspath-activity-selector');
    let activityStarting = document.getElementById('progresspath-activity-starting');
    let activityTarget = document.getElementById('progresspath-activity-target');
    let activityHiddenWarning = document.getElementById('progresspath-activity-hidden-warning');
    let advancedSettingsIcon = document.getElementById('progresspath-advanced-settings-icon');
    ...
    if (activitySelector) {
        ...
        activitySelector.addEventListener('change', function() {
            ...
            if (activitySelector.value) {
                ...
                document.getElementById(elementForActivitySelector).classList.remove('progresspath-emptyplace');
            } else {
                document.getElementById(elementForActivitySelector).classList.add('progresspath-emptyplace');
            }
            updateActivities();
            updateCode();
        });
    ...
    if (advancedSettingsIcon) {
        let advancedSettings = document.getElementById('progresspath-advanced-settings');
        ...
        let advancedSettingsClose = document.getElementById('progresspath-advanced-settings-close');
    ...
    function hideContextMenu() {
        ...
        if (e) {
            e.classList.remove('progresspath-selected-activity-selector');
        }
    ...
    function makeDraggable(el) {
        ...
        function startDrag(evt) {
            ...
            if (evt.target.classList.contains('progresspath-draggable')) {
        ...
        function startTouch(evt) {
            ...
            if (
                evt.target.classList.contains('progresspath-draggable') ||
        ...
        function updatePathDeclaration(oldDefinition, targetX, targetY, targetP = targetPoints.firstPoint) {
            ...
            // The d attribute of an SVG path in a progresspath can have two different formats (in this version):
    ...
    function dblclickHandler(event) {
        ...
        hideAdvancedSettings();
        unselectAll();
        if (event.target.classList.contains('progresspath-mapcontainer') ||
            event.target.classList.contains('progresspath-background-image')) {
            addPlace(event);
        } else if (event.target.classList.contains('progresspath-place')) {
        ...
        } else if (event.target.classList.contains('progresspath-path')) {
    ...
    function clickHandler(event) {
        ...
        hideContextMenu();
        hideAdvancedSettings();
        if (event.target.classList.contains('progresspath-place') && selectedElement === null) {
            if (firstPlace === null) {
                firstPlace = event.target.id;
                document.getElementById(firstPlace).classList.add('progresspath-selected');
            } else {
                ...
                if (first) {
                    first.classList.remove('progresspath-selected');
                }
                firstPlace = null;
    ...
    /**
     * Removes the classes 'progresspath-selected' and 'progresspath-selectet-activity-selector'
     * from all nodes
     */
    function unselectAll() {
        Array.from(document.getElementsByClassName('progresspath-selected')).forEach(function(e) {
            e.classList.remove('progresspath-selected');
        });
        Array.from(document.getElementsByClassName('progresspath-selected-activity-selector')).forEach(function(e) {
            e.classList.remove('progresspath-selected-activity-selector');
        });
    }
    ...
    function refreshBackgroundImage() {
        ...
        if (previewimage.length > 0) {
            let background = document.getElementById('progresspath-background-image');
    ...
    function updateActivities() {
        ...
        options.forEach(function(n) {
            if (activities.includes(n.value)) {
                n.classList.add('progresspath-used-activity');
                if (n.selected) {
                ...
                n.classList.remove('progresspath-used-activity');
            }
        });
    }
